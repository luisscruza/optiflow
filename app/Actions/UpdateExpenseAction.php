...
{
    /**
     * @param  array{workspace_id:int,contact_id:int,document_number:string,issue_date:string,subtotal_amount:numeric-string|int|float,itbis_amount:numeric-string|int|float,isc_amount:numeric-string|int|float,withheld_itbis_amount:numeric-string|int|float,withheld_isr_amount:numeric-string|int|float,is_informal:bool,status:string,notes?:string|null,attachments?:array<UploadedFile>,remove_attachment_ids?:array<int>}  $data
     */
    public function handle(User $user, Expense $expense, array $data): Expense
    {
        $workspace = $this->resolveWorkspace($user, (int) $data['workspace_id']);

        return DB::transaction(function () use ($expense, $workspace, $data): Expense {
            $expense->update([
            ...
            $this->removeAttachments($expense, $data['remove_attachment_ids'] ?? []);
            $this->addAttachments($expense, $data['attachments'] ?? []);

            return $expense->fresh(['contact', 'workspace', 'media']);
        });
    }
     ...
     */
    private function removeAttachments(Expense $expense, array $attachmentIds): void
    {
        if ($attachmentIds === []) {
            return;
        }
