import { Package, ArrowLeft, Calendar, User, TrendingUp, TrendingDown, RotateCcw, ArrowRightLeft, Building2 } from 'lucide-react';
...
import { type BreadcrumbItem, type Product, type ProductStock, type PaginatedStockMovements, type Workspace } from '@/types';
...
export default function StockAdjustmentsShow({ product, currentStock, stockHistory, workspace }: Props) {
  ...
  const getMovementBadge = (type: string) => {
    switch (type) {
      ...
      case 'remove_quantity':
        return <Badge variant="destructive">Remove Quantity</Badge>;
      case 'set_quantity':
        return <Badge variant="outline">Set Quantity</Badge>;
      case 'transfer_in':
        return <Badge variant="default">Transfer In</Badge>;
      case 'transfer_out':
        return <Badge variant="destructive">Transfer Out</Badge>;
      default:
      ...
      <div className="max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-muted-foreground">
              Stock history for {product.sku} in {workspace.name}
            </p>
          </div>
          <Button asChild>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {currentStock ? formatQuantity(currentStock.quantity) : '0'}
              </div>
              {status && (
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {currentStock ? formatQuantity(currentStock.minimum_quantity) : '0'}
              </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Workspace Transfer</TableHead>
                  ...
                  {stockHistory.data.length === 0 ? (
                    ...
                    stockHistory.data.map((movement) => (
                      <TableRow key={movement.id}>
                        ...
                        <TableCell>
                          {renderWorkspaceTransfer(movement)}
                        </TableCell>
                        ...
                        <TableCell>
                          <div className="flex items-center space-x-1">
                            ...
                            <span className="text-sm">
                              {movement.created_by?.name || 'Unknown'}
                            </span>
                          </div>
                        </TableCell>
                        <TableCell className="text-muted-foreground">
            ...
            {stockHistory.last_page > 1 && (
              <div className="flex items-center justify-between space-x-2 py-4">
                <div className="text-sm text-muted-foreground">
                  Showing {stockHistory.from} to {stockHistory.to} of{' '}
                ...
                <div className="flex space-x-2">
                  {stockHistory.links.prev && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => window.location.href = stockHistory.links.prev!}
                    >
                      Previous
                    </Button>
                  )}
                  {stockHistory.links.next && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => window.location.href = stockHistory.links.next!}
                    >
                      Next
                    </Button>
                  )}
