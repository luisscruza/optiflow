...
interface Props {
    ...
    contact_relationships: Array<{ related_contact_id: number; related_contact_name: string; description?: string | null }>;
}
...
export default function EditContact({ contact, contact_types, identification_types, relationshipSearchResults = [], contact_relationships }: Props) {
    ...
    const [relationshipLabels, setRelationshipLabels] = useState<Record<string, string>>(() => {
        ...
        contact_relationships.forEach((r) => {
            labels[r.related_contact_id.toString()] = r.related_contact_name;
        });
        return labels;
    });
    ...
    const { data, setData, put, processing, errors } = useForm<ContactFormData>({
        ...
        address: {
            ...
            description: contact.primary_address?.description || '',
        },
    ...
    const relationshipOptions: ServerSearchableSelectOption[] = (relationshipSearchQuery.length >= 2 ? relationshipSearchResults : [])
        .filter((c) => !data.relationships.some((r) => r.related_contact_id === c.id.toString()))
        .map((c) => ({
            value: c.id.toString(),
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
                <div className="mb-6">
                    <h1 className="text-xl font-semibold tracking-tight">Editar contacto</h1>
                    <p className="text-sm text-muted-foreground">Actualiza la informacion de {contact.name}.</p>
                </div>
