    ...
    return (
        <div className="w-full">
            <div className="rounded-lg border border-gray-200 p-3">
                <h3 className="mb-3 border-b border-gray-200 pb-2 text-sm font-semibold text-gray-900">Oftalmoscopía</h3>

                {/* Content */}
                <div className="min-h-[120px] flex-1 overflow-y-auto">
                    {hasData ? (
                        <div className="flex min-h-[120px] flex-col justify-start space-y-2">
                            {/* Pupilar Exam Summary */}
                            {(pupilarOdValues.length > 0 || pupilarOiValues.length > 0) && (
                                <>
                                    <div className="space-y-1">
                                        <p className="text-xs font-semibold text-purple-700">Reflejos Pupilares:</p>
                                    </div>

                                    {pupilarOdValues.length > 0 && (
                                        <div className="space-y-1">
                                            <p className="text-xs font-semibold text-gray-900">OD:</p>
                                            <div className="border-l-2 border-purple-100 pl-2">
                                                <p className="line-clamp-1 text-xs leading-tight text-gray-700">{pupilarOdValues.join(', ')}</p>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {pupilarOiValues.length > 0 && (
                                        <div className="space-y-1">
                                            ...
                                            <div className="border-l-2 border-purple-100 pl-2">
                                                <p className="line-clamp-1 text-xs leading-tight text-gray-700">{pupilarOiValues.join(', ')}</p>
                                            </div>
                                        </div>
                                    )}
                                </>
                            ...
                            {(oftalmoscopiaOdValues.length > 0 || oftalmoscopiaOiValues.length > 0) && (
                                <>
                                    ...
                                    {oftalmoscopiaOdValues.length > 0 && (
                                        <div className="space-y-1">
                                            ...
                                            <div className="border-l-2 border-blue-100 pl-2">
                                                <p className="line-clamp-2 text-xs leading-tight text-gray-700">{oftalmoscopiaOdValues.join(', ')}</p>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {oftalmoscopiaOiValues.length > 0 && (
                                        <div className="space-y-1">
                                            <p className="text-xs font-semibold text-gray-900">OI:</p>
                                            <div className="border-l-2 border-green-100 pl-2">
                                                <p className="line-clamp-2 text-xs leading-tight text-gray-700">{oftalmoscopiaOiValues.join(', ')}</p>
                                            </div>
                                        </div>
                                    )}
                                </>
                            ...
                            {data.oftalmoscopia_observaciones?.trim() && (
                                <div className="space-y-1">
                                    ...
                                    <div className="border-l-2 border-amber-100 pl-2">
                                        <p className="line-clamp-2 rounded bg-gray-50 p-1 text-xs leading-tight text-gray-700">
                        ...
                        <div className="flex min-h-[120px] flex-col items-center justify-center text-center">
                            ...
                            <p className="text-xs text-gray-500">No se han completado campos de oftalmoscopía</p>
                        </div>
                    )}
                </div>

                {/* Footer */}
                <div className="flex justify-center border-t border-gray-100 bg-gray-50 px-3 py-2">
                    <Dialog open={isOpen} onOpenChange={setIsOpen}>
                        <DialogTrigger asChild>
                            <Button
                                variant="outline"
                                size="sm"
                                className="h-6 w-6 border-gray-300 p-0 transition-all duration-200 hover:border-blue-400 hover:bg-blue-50 hover:shadow-sm"
                            >
                        ...
                        <DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto">
                            <DialogHeader>
                                <DialogTitle className="flex items-center gap-2">
                                    ...
                                    Reflejos Pupilares / Oftalmoscopía
                                </DialogTitle>
                            </DialogHeader>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-4">
                                    <h3 className="border-b border-gray-200 pb-2 text-lg font-semibold text-gray-900">Reflejos Pupilares</h3>
                                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                                        {/* Ojo Derecho */}
                                        <div className="space-y-4">
                                            <h4 className="text-md border-b border-gray-300 pb-1 font-medium text-gray-800">Ojo Derecho</h4>
                                                ...
                                                {pupilarFields.map((field) => (
                                                    <div key={field.key} className="space-y-2">
                                                        ...
                                                        <Input
                                                            ...
                                                            onChange={(e) => onChange(`pupilar_od_${field.key}`, e.target.value)}
                                                            className="border-gray-300 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                                            placeholder={`Ingrese ${field.label.toLowerCase()}`}
                                                        />
                                                        {errors?.[`pupilar_od_${field.key}`] && (
                                                            <p className="text-xs text-red-600">{errors[`pupilar_od_${field.key}`]}</p>
                                                        )}
                                                    </div>
                                                ))}
                                            </div>
                                        ...
                                        {/* Ojo Izquierdo */}
                                        <div className="space-y-4">
                                            <h4 className="text-md border-b border-gray-300 pb-1 font-medium text-gray-800">Ojo Izquierdo</h4>
                                            <div className="space-y-3">
                                                {pupilarFields.map((field) => (
                                                    <div key={field.key} className="space-y-2">
                                                        <Label className="text-sm font-medium text-gray-700">{field.label}</Label>
                                                        <Input
                                                            ...
                                                            onChange={(e) => onChange(`pupilar_oi_${field.key}`, e.target.value)}
                                                            className="border-gray-300 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                                            placeholder={`Ingrese ${field.label.toLowerCase()}`}
                                                        />
                                                        {errors?.[`pupilar_oi_${field.key}`] && (
                                                            <p className="text-xs text-red-600">{errors[`pupilar_oi_${field.key}`]}</p>
                                                        )}
                                                    </div>
                                                ))}
                                            </div>
                                ...
                                {/* Oftalmoscopía */}
                                <div className="space-y-4">
                                    <h3 className="border-b border-gray-200 pb-2 text-lg font-semibold text-gray-900">Oftalmoscopía</h3>
                                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                                        {/* Ojo Derecho */}
                                        <div className="space-y-4">
                                            <h4 className="text-md border-b border-gray-300 pb-1 font-medium text-gray-800">Ojo Derecho</h4>
                                                ...
                                                {oftalmoscopiaFields.map((field) => (
                                                    <div key={field.key} className="space-y-2">
                                                        ...
                                                        {errors?.[`oftalmoscopia_od_${field.key}`] && (
                                                            <p className="text-xs text-red-600">{errors[`oftalmoscopia_od_${field.key}`]}</p>
                                                        )}
                                                    </div>
                                                ))}
                                            </div>
                                                ...
                                                {oftalmoscopiaFields.map((field) => (
                                                    <div key={field.key} className="space-y-2">
                                                        ...
                                                        {errors?.[`oftalmoscopia_oi_${field.key}`] && (
                                                            <p className="text-xs text-red-600">{errors[`oftalmoscopia_oi_${field.key}`]}</p>
                                                        )}
                                                    </div>
                                                ))}
                                            </div>
                                ...
                                {/* Observaciones */}
                                <div className="space-y-3">
                                    <h3 className="border-b border-gray-200 pb-2 text-lg font-semibold text-gray-900">Observaciones</h3>
                                    <div className="space-y-2">
                                        <Textarea
                                            ...
                                            className="min-h-[100px] w-full border-gray-300 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                            rows={4}
                                        />
                                        {errors?.oftalmoscopia_observaciones && (
                                            <p className="text-xs text-red-600">{errors.oftalmoscopia_observaciones}</p>
                                        )}
                                    </div>
                                </div>
